#!/usr/bin/env bash
# Keep the recall index fresh in the background on session start.
#
...
#
# So: if `recall` is installed, spawn a fully-detached incremental index and exit 0.

command -v recall >/dev/null 2>&1 || exit 0
nohup recall index --incremental </dev/null >/dev/null 2>&1 & disown 2>/dev/null || true
exit 0
